...
6. Known, deferred watchdog hazards:
   - (M4) `destroy_verified` destroys before it lists, so destroying an already-gone instance raises and the lease is never cleared (`drain_running` stays true and the bot refuses the next `/confirm`) — same shape on RunPod — **Done in Plan 2 (2026-09-19):** a watchdog that destroys a gone instance then re-lists counts it gone; `vast_rent.py`'s `rent()` now verifies destruction by re-listing.
...
7. Two parked Makefile-verify minors — **Done in Plan 2 (2026-09-19):** (a) an exit-0 output from `vastai show instances-v1` that does not contain the token `"instances"` is now treated as "could not verify" and never clears `.env`; matching on `"instances": [` (to handle error text that itself contains the quoted token) is still parked; (b) `vastai show instances-v1` output can contain whitespace in the `id` field; commit 3ab85d8 applied `$(strip …)` to the id read from `.env` in the Makefile recipe, and the recipe still greps the listing for `"id": *<id>`.
