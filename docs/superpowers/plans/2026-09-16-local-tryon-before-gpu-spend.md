...
> **Amended during execution.** This task's code blocks below are what was *specified*, not what shipped. Its review found five Important defects and the fix round changed four things materially; the ledger records each with its ruling. Summary, so a later reader does not trust the blocks below as the final shape:
>
> 1. The exit-3 branch must `path.unlink(missing_ok=True)` **after** `_offer_run_confirm` returns. As written below it never unlinks, so on the plan's headline path — no restart required — `animating = _progress_path(...).exists()` stays True forever, pinning the poll loop at 2s (~30 Telegram calls/min instead of ~1.2) and freezing a message that keeps claiming a try-on is running. The `offered` latch stays, but only for the crash window between latch-write and render; the unlink is what actually stops the re-send, and the docstring must say so.
> 2. A restart mid-Phase A orphans the file permanently: `_PHASE_A` and `_PHASE_A_RC` are in-memory, so `phase_a_exit` returns None forever, `tick_phase_a` bails, `tick_progress` bails on the ownership guard, and nothing ever clears the file. Recovery is to unlink and say honestly that the bot restarted and lost track, that nothing was rented, and that N/M finished try-ons are preserved so Run will skip them — never that the child died, because a hand-run bot's child may still be alive. Detection (`phase == "local"` ∧ not running ∧ exit is None) is *also* true in the window between `_start_progress(phase="local")` and `start_phase_a`, so gate recovery on a durable `seen_running` flag set in the redraw branch, or the recovery unlinks a file for a Phase A that is about to start.
> 3. The spend button carries `_run_token(chat_id)`, **not** the manifest stem as written below. The stem version's premise — "the manifest is not rewritten between the panel and the tap" — is not an invariant: `_job_manifest_path(chat_id)` is the very file `_run_token` stamps and `_maybe_show_manifest` rewrites, and once Phase A has exited `busy()` is False so nothing blocks that rewrite. `_run_token`'s own docstring names the harm ("tapping it would spend $0.99/hour on inputs they never reviewed"), and `_CB_PHASE_A_REUSE`/`_RERUN` carry the token for exactly this reason. The handler derives the path from `_job_manifest_path(chat_id)`, the way `_CB_RUN_GO` does. `_do_resume`'s concurrency gate also moves from `drain_running` to `busy`, which `busy`'s own docstring says is the right predicate for "drain.py reads this manifest" and which pre-empts a two-writer race on `batch/<name>.state.json` once Task 10 launches Phase A.
> 4. `/status` must pass the payload's `phase` through to `progress_text`. It already parses that payload; omitting `phase` makes it render "waiting for the pod — nothing recorded yet" during Phase A, which is the exact inference §6.5 exists to remove, on a renderer whose own comment promises it "can never disagree with what is already on screen".
>
> Also: `test_still_running_only_redraws` as written below asserts on `tg.buttons[-1]`, which is `[None]` after `_start_progress` (it sends no keyboard, and `edit_message` appends to `edit_buttons`) — so it cannot fail. It must assert on `tg.edits[-1][1]` and use `tg.callback_data()`, which spans sends *and* edits and exists for precisely this trap.
