...
Date: 2026-09-24 · Status: implemented on `feat/phase-6-follow-ups`, implemented, reviewed, merged and deployed — PR #68, on `main` as `f0b7fa7` · Branch: `feat/phase-6-follow-ups` (one branch, one PR)
...
| VPS pre-merge check (drain / Phase A / lease / migration) | **clear.** VPS on `f65fc63`; `pgrep -af "[d]rain.py\|[b]atch_run.py\|[p]hase_a"` → none; `motion-bot` active since 03:23:59 UTC; `GPU_INSTANCE_ID` count 0, so no lease; no `migrate-progress.json`; and `git ls-files` matched no `batch/tg-*.last-kill.json` or `*.confirmed-generation.json`, so nothing of that shape was ever tracked there either | 2026-09-24 | controller, read-only over `doctl compute ssh` |
| `deploy-bot` workflow run | **run `36029721301`, success in 16 s**, 4 s after the merge. PR #68 squash-merged as `f0b7fa7` at 16:46:40Z; `origin/main`'s tree and branch tip `e39d2b5`'s tree are both `37761d39ea003c86361cbdba6af82ff4295a8bdd`, so the squash lost nothing. VPS verified on `f0b7fa7` with `motion-bot` active since 16:46:56 UTC and the gate present in the deployed `bot.py` | 2026-09-24 | controller |
| `make ios-contract` (post-deploy, live) | **exit 0, 14/14 ok** against the VPS on `f0b7fa7`. Same fourteen routes as the pre-deploy run, which is the other half of §3's deploy-order property: the new server answers an unchanged contract. Still no fifteenth route, because this branch adds none | 2026-09-24 | controller, live against the VPS |
