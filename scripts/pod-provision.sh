...
$(warn "Dry run — nothing rented.")
  ...
  Hết máy '$GPU' là dừng ở đây — không tự xoay sang card khác. Đổi GPU trong .env rồi chạy lại.
...
EOF
  ...
  RAW="$("${CREATE[@]}" 2>&1)" || {
    if printf '%s' "$RAW" | grep -qiE "no instances available|no longer any instances|out of (stock|capacity)|insufficient capacity"; then
      die "hết máy '$GPU' tại datacenter của volume — không tự xoay sang card khác.
  Đổi GPU trong .env rồi chạy lại, hoặc chờ. Xem còn con nào:  runpodctl gpu list -o json | grep -A3 EU-RO-1"
    fi
    die "runpodctl pod create failed ('$GPU'):
$RAW"
  }
  NEW_ID="$(printf '%s' "$RAW" | rp_pick id,podId)"
  if [ -n "$NEW_ID" ]; then
